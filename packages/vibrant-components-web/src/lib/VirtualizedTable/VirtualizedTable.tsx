...
function extractTextFromReactElement(element: React.ReactNode): string {
...
export const VirtualizedTable = <Data extends Record<string, any>, RowKey extends keyof Data>({
  ...
  useEffect(() => {
    ...
    const handleSelectStart = (event: Event) => {
      if (multiCellSelectable && tableEl.contains(event.target as Node)) {
  ...
  const copySelectedCells = useCallback(() => {
    ...
    for (let rowIdx = startRow; rowIdx <= endRow; rowIdx++) {
      ...
      const rowCells = columns.slice(startCol, endCol + 1).map(column => {
        ...
        if (column.renderDataCell) {
          const cellEl = column.renderDataCell?.(row);

          return extractTextFromReactElement(cellEl);
        }
  ...
  const components: TableComponents<Data, any> = useMemo(
    () => ({
      Table: ({ style, ...props }) => (
        <table tabIndex={0} {...props} style={{ ...style, width: '100%', tableLayout }} data-testid={testId} />
      ),
    }),
    [testId, tableLayout]
  );
