...
export default async function (tree: Tree, options: GenerateStoryImportGeneratorSchema) {
  const storyFileNames = sync(options.stories, { absolute: false, cwd: tree.root, ignore: ['**/node_modules'] }).map(
    pathName => path.relative(path.dirname(options.outputFile), pathName)
  );
