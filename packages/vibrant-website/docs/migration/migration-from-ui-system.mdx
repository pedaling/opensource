...
## Migrate `<SafeAreaContent />` to `<SafeAreaView />`
...
<img src="/img/docs/textfield-dynamic-label.gif" />
...
Icon 컴포넌트에 `type` 속성이 제거되고 `<Icon.Play.Regular />`와 같이 별도의 컴포넌트로 관리합니다.
